          ...
          function  recherche($tab,$id)
                ...
                foreach ($jour as $key) {
                 ...
                 foreach ($planifications as $row) {
                          ...
                          if ($row->jour_id==$value->id_jour && ($row->type_planing != "BIBLIOTHEQUE" && $row->type_planing != "Congé" && $row->type_planing != "INVEST_HUMAIN")) {
                ...
                echo "<tr id='".$value->id_jour."'><th rowspan='".$value->c."' style='height: 5%; width: 7%' width='100px''><center><b>".$value->intitule_jour."</b></center></th>";
                     ...
                     $salle_id=0;
                        foreach ($planifications as $row) {
                          ...
                          if ( ($row->type_planing != "BIBLIOTHEQUE" && $row->type_planing != "Congé" && $row->type_planing != "INVEST_HUMAIN") 
                                ...
                                && ($row->plages[0]->heure_debut[0]+$row->plages[0]->heure_debut[1])==$min_jour && recherche($id_planification,$row->id_plannification)) 
                          {
                            /*echo "<pre>";
                    ...
                    if($trouve!=1){
                     ...
                     //$trouve=0;   
                                    ...
                                    $salle_id=0;
                        foreach ($planifications as $row) {
                           if ( ($row->type_planing != "BIBLIOTHEQUE" && $row->type_planing != "Congé" && $row->type_planing != "INVEST_HUMAIN") && $row->jour_id==$value->id_jour && $plage!=$row->plages[0]->id_plage && $salle_id!=$row->salle_id && ($row->plages[0]->heure_debut[0].$row->plages[0]->heure_debut[1])==$min_jour && recherche($id_planification,$row->id_plannification) ){

                             $trouve=1;
                              $id_planification[$p]=$row->id_plannification;
                             $p+=1;
                            echo "<td colspan='".$row->plages[0]->duree."' align='center'><font size='1'>".$row->ec[0]->intitule_ec."<br>".$row->evolution."/".$row->ec[0]->heure."<br>"."<b>".$row->niv."</b>".$row->salle."</font></td>";
                            $j+=(int)$row->plages[0]->duree-1;
                             $min_jour+=$j;
                            $plage=$row->plages[0]->id_plage;
                            $salle_id=$row->salle_id;
